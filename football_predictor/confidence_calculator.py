...
class ConfidenceCalculator:
    ...
    def calculate_outcome_specific_confidence(self, probabilities, home_data, away_data, inputs):
        ...
        # 🚨 CRITICAL: Determine predicted winner FIRST
        ...
        # Store current probabilities for confidence calculation
        home_data['current_probability'] = probabilities['home_win']
        away_data['current_probability'] = probabilities['away_win']
        home_data['is_predicted_winner'] = (predicted_outcome == 'home_win')
        away_data['is_predicted_winner'] = (predicted_outcome == 'away_win')
        
        # Outcome-specific adjustments
        home_win_confidence = self._calculate_win_confidence(
            probabilities['home_win'], context_confidence, home_data, away_data, 'home', predicted_outcome
        )
        
        away_win_confidence = self._calculate_win_confidence(
            probabilities['away_win'], context_confidence, home_data, away_data, 'away', predicted_outcome
        )
        
        draw_confidence = self._calculate_draw_confidence(
            probabilities['draw'], context_confidence, home_data, away_data, predicted_outcome
        )
        ...
        # 🚨 CRITICAL SANITY CHECK: Predicted winner MUST have highest confidence
        predicted_confidence = outcome_confidences[predicted_outcome]
        other_confidences = [v for k, v in outcome_confidences.items() if k != predicted_outcome]
        max_other_confidence = max(other_confidences) if other_confidences else 0
        
        if predicted_confidence <= max_other_confidence:
            print(f"🚨 CONFIDENCE FIX: Boosting {predicted_outcome} from {predicted_confidence} to {max_other_confidence + 8}")
            outcome_confidences[predicted_outcome] = max_other_confidence + 8
        
        # Final bounds check
        for outcome in outcome_confidences:
            outcome_confidences[outcome] = min(85, max(35, outcome_confidences[outcome]))
    ...
    def _calculate_win_confidence(self, win_probability, context_confidence, team_data, opponent_data, side, predicted_outcome):
        """🚨 COMPLETELY REVISED: Logical confidence based on probability"""
        
        # 🚨 BASE CONFIDENCE: Direct correlation with probability
        ...
        # 🚨 PREDICTED WINNER BOOST
        is_predicted_winner = (
            (side == 'home' and predicted_outcome == 'home_win') or
            (side == 'away' and predicted_outcome == 'away_win')
        )
        
        if is_predicted_winner:
            # Significant boost for predicted winner
            boost = max(10, (win_probability - 0.35) * 40)  # 10-20% boost
            base_confidence += boost
            print(f"🔍 PREDICTED WINNER BOOST: {side} +{boost:.1f}%")
        
        # 🚨 ELITE TEAM BOOST
        team_tier = team_data['base_quality']['structural_tier']
        opponent_tier = opponent_data['base_quality']['structural_tier']
        
        if team_tier == 'elite' and opponent_tier != 'elite' and win_probability > 0.40:
            elite_boost = 8
            base_confidence += elite_boost
            print(f"🔍 ELITE TEAM BOOST: {side} +{elite_boost}%")
        ...
        if side == 'home':
            ...
            if home_adv['strength'] == 'strong':
                base_confidence += 5
                print(f"🔍 STRONG HOME ADVANTAGE: +5%")
            elif home_adv['strength'] == 'weak':
                base_confidence -= 3
        
        # Apply context (limited influence)
        final_confidence = 0.85 * base_confidence + 0.15 * context_confidence
        
        return min(85, max(35, final_confidence))

    def _calculate_draw_confidence(self, draw_probability, context_confidence, home_data, away_data, predicted_outcome):
        """🚨 FIXED: Conservative draw confidence"""
        ...
        # 🚨 Draw confidence penalty
        base_confidence *= 0.85  # 15% reduction for draw uncertainty
        
        # Small boost if draw is predicted
        if predicted_outcome == 'draw':
            base_confidence += 5
            print(f"🔍 PREDICTED DRAW BOOST: +5%")
        
        # Close matches increase draw confidence
        ...
        final_confidence = 0.9 * base_confidence + 0.1 * context_confidence
        
        return min(65, max(25, final_confidence))
    ...
    def calculate_goal_market_confidence(self, total_goals, probability, market_type="over_2.5"):
        """Calculate confidence for goal markets"""
        if market_type == "over_1.5":
            if probability >= 0.90: confidence = 82
            elif probability >= 0.80: confidence = 75
            elif probability >= 0.65: confidence = 68
            elif probability >= 0.50: confidence = 60
            else: confidence = 50
        elif market_type == "over_2.5":
            if probability >= 0.80: confidence = 78
            elif probability >= 0.70: confidence = 72
            elif probability >= 0.55: confidence = 65
            elif probability >= 0.40: confidence = 58
            else: confidence = 48
        else:  # over_3.5
            if probability >= 0.70: confidence = 75
            elif probability >= 0.60: confidence = 68
            elif probability >= 0.45: confidence = 60
            elif probability >= 0.30: confidence = 52
            else: confidence = 42
        
        # Adjust based on total goals expectation
        if total_goals > 4.0 and probability > 0.6: confidence += 3
        elif total_goals < 2.0 and probability < 0.4: confidence += 2
    ...
    def calculate_btts_confidence(self, btts_probability, home_data, away_data, home_goal_exp, away_goal_exp):
        """🚨 COMPLETELY FIXED: BTTS confidence with proper logic"""
        
        # 🚨 CRITICAL: Confidence based on distance from 50%
        distance_from_50 = abs(btts_probability - 0.5)
        
        if distance_from_50 > 0.25:  # Very clear signal
            base_confidence = 70 + (distance_from_50 - 0.25) * 100  # 70-75%
        elif distance_from_50 > 0.15:  # Clear signal
            base_confidence = 60 + (distance_from_50 - 0.15) * 100  # 60-70%
        elif distance_from_50 > 0.08:  # Moderate signal
            base_confidence = 50 + (distance_from_50 - 0.08) * 142  # 50-60%
        elif distance_from_50 > 0.03:  # Weak signal
        ...
        # Reduce confidence if historical data is inconsistent
        if abs(hist_avg - btts_probability) > 0.2:
            base_confidence *= 0.85
        
        # 🚨 DIFFERENT CONFIDENCE FOR YES/NO
        # The more likely outcome gets slightly higher confidence
        confidence_yes = base_confidence
        confidence_no = base_confidence
        
        if btts_probability > 0.51:  # BTTS Yes more likely
            confidence_yes += 3
            confidence_no -= 2
        elif btts_probability < 0.49:  # BTTS No more likely
            confidence_no += 3
            confidence_yes -= 2
        
        print(f"🔍 BTTS CONFIDENCE: Yes={confidence_yes:.0f}%, No={confidence_no:.0f}% (prob={btts_probability:.1%})")
        
        # Return the confidence for the "Yes" outcome (engine will calculate "No" as needed)
        return min(75, max(35, confidence_yes))
